"""CLI runner for example 03: typed invoice extraction."""
...
def _score_wrapper(metric):
    """dspy.Evaluate needs a float-like score. Accept Prediction / dict / float."""
...
def cmd_dry_run(_):
    ...
    # Metric probe with a synthetic Pydantic output.
    fake_record = pipeline.InvoiceRecord(
        vendor="Acme Supplies Co.",
        date="2024-03-15",
        line_items=[
            pipeline.LineItem(description="Widget", quantity=10, unit_price=5.0)
        ],
        total=102.06,
    )
    fake = dspy.Prediction(record=fake_record)
...
def cmd_optimize(args):
    ...
    optimizer = dspy.GEPA(
        ...
        seed=args.seed,
        # Typed Pydantic output signatures are dynamic classes that stock
        # pickle cannot resolve by attribute lookup when GEPA saves state.
        # cloudpickle handles dynamic/generated classes correctly.
...
def _write_results_md(r: dict) -> None:
    ...
    RESULTS_MD.write_text(
        f"""# Example 03 — Invoice Extraction Results

| Metric | Baseline | Optimized | Δ |
|---|---:|---:|---:|
| Weighted multi-axis | {r["baseline_score"]:.3f} | {r["optimized_score"]:.3f} | **{delta:+.3f}** |
